...
// Check for disabled account error message on login page
if (window.location.pathname.endsWith('index.html') || window.location.pathname === '/' || window.location.pathname === '') {
  const urlParams = new URLSearchParams(window.location.search);
  if (urlParams.get('error') === 'disabled') {
    // Show error message for disabled account
    const errorLoginEmail = document.getElementById("error-login-email");
    if (errorLoginEmail) {
      errorLoginEmail.textContent = "Your account has been disabled. Please contact the administrator.";
      errorLoginEmail.style.display = "block";
      const emailInput = document.getElementById("login-email");
      if (emailInput) {
        emailInput.style.border = "1px solid #d32f2f";
      }
    }
...
async function checkEmailExists(email) {
  try {
    ...
    // On other errors (network issues, etc.), return true to proceed with login attempt
    // This prevents the email check from blocking valid login attempts
    return true;
  }
}
    ...
    if (!user) {
      ...
      try {
        ...
        if (userDocSnap.exists()) {
          ...
          if (userData.status === 'inactive') {
            console.warn('⚠️ User account is disabled, logging out...');
...
if (window.location.pathname.endsWith('tents-chairs-request.html') || window.location.pathname.endsWith('/tents-chairs-request')) {
  ...
  async function submitTentsChairsRequest(data) {
    ...
    try {
      ...
      // � CHECK IF USER ACCOUNT IS DISABLED
      console.log('🔒 [Tents/Chairs Form] Checking user account status...');
      const userDocRef = doc(db, 'users', user.uid);
      const userDocSnap = await getDoc(userDocRef);
...
if (window.location.pathname.endsWith('conference-request.html') || window.location.pathname.endsWith('/conference-request')) {
  ...
  async function submitToFirestore(formData) {
    ...
    try {
      ...
      // � CHECK IF USER ACCOUNT IS DISABLED
      console.log('🔒 [Conference Form] Checking user account status...');
      const userDocRef = doc(db, 'users', user.uid);
      const userDocSnap = await getDoc(userDocRef);
...
if (window.location.pathname.endsWith('admin-user-manager.html') || window.location.pathname.endsWith('/admin-user-manager')) {
   ...
   * Handles the Review Requests dropdown toggle
   */
  function setupSidebarDropdownsUM() {
    const reviewRequestsToggle = document.getElementById('reviewRequestsToggle');
